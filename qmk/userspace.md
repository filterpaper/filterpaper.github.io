...
Exported key map from the [Configurator](https://config.qmk.fm/) may not be favorable to power users that prefer hand editing key codes `keymap.c` format. Key map wrappers can be adapted to use with json files. Instead of exporting a layout from the Configurator, create one that reference a *macro* name for each row, like this example of `planck.json`:
```json
{
    "author": "",
...
```
The macro names `BASE1` and `LOWER1` is your own choosing, pick anything that make sense. Create a `wrappers.h` to map those macro names to the actual key layout in text:
```c
#pragma once
...
#define PLANCKBASE KC_DEL,  KC_LALT, KC_LCTL, KC_LGUI, LT(2,KC_SPC), KC_SPC, KC_SPC, LT(3,KC_SPC), KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT
...
```
qmk flash ~/qmk_firmware/users/newbie/placnk.json
```
